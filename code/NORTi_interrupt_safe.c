...
static void NORTi_INT_HOOK_inline_asm_dummy(void);

void NORTi_INT_HOOK_init(void)
{
    set_intb((void *)Int_Hook_Vectors);

    NORTi_INT_HOOK_inline_asm_dummy();
}
...
{
    XCHG [SP], R15 // address of hook_entry_XXX + 3 --> R15 // 3 = size of BSR.W
    ADD #(- (__$hook_entry___0 + 3) + TOPOF C$VECT), R15 // i.e. (top of VECT) + (hook_entry_XXX - hook_entry___0) --> R15
    MOV.L [R15], R15
    XCHG [SP], R15
    RTS // i.e. JMP [SP+]
}

#pragma inline_asm INT_COMMON_nonOS_POPM_R14R15_SETPSW_I
static void INT_COMMON_nonOS_POPM_R14R15_SETPSW_I(void)
{
    POPM R14-R15
...
static unsigned long INT_COMMON_NORTi_hook_entry_addr;
...
static void INT_COMMON_NORTi(void)
{
    ...
    MVFC PSW, R15
#if defined(__RXV1) || defined(__RXV2)
    SHLR #24, R15
    AND #15, R15
#else
    BFMOVZ #24, #0, #4, R15, R15
#endif
    MOV.L #_NORTi_INT_HOOK_KNL_LEVEL, R14
    CMP [R14].UB, R15
    BGTU __$INT_COMMON_nonOS_POPM_R14R15_SETPSW_I // if (NORTi_INT_HOOK_KNL_LEVEL < PSW.IPL) goto INT_COMMON_nonOS_POPM_R14R15_SETPSW_I

    MOV.L #__$INT_COMMON_NORTi_hook_entry_addr, R15
    MOV.L 8[SP], [R15] // address of hook_entry_XXX + 3 --> INT_COMMON_NORTi_hook_entry_addr // 3 = size of BSR.W

    POPM R14-R15
    ADD #4, SP

    BSR.A _v3_ent_int // Call NORTi API. The task context is saved in the API.

    MOV.L #__$INT_COMMON_NORTi_hook_entry_addr, R15
    MOV.L [R15], R15 // address of hook_entry_XXX + 3 --> R15 // 3 = size of BSR.W
    ADD #(- (__$hook_entry___0 + 3) + TOPOF C$VECT), R15 // i.e. (top of VECT) + (hook_entry_XXX - hook_entry___0) --> R15
    MOV.L [R15], R15
    PUSHC PSW
    MOV.L #?+, R14
...
?:
    ...
    BRA.A _v3_ret_int // Call NORTi API (in this case, jump to NORTi). The task context is restored in the API.
}
...
extern uint8_t NORTi_INT_HOOK_KNL_LEVEL;

// To prevent symbols and the section name symbol `C$VECT` from being optimized out.
void * const NORTi_INT_HOOK_inline_asm_sym_dummy[] = {
    (void *)v3_ent_int,
    (void *)v3_ret_int,
    (void *)INT_COMMON_nonOS,
    (void *)INT_COMMON_NORTi,
    (void *)INT_COMMON_nonOS_POPM_R14R15_SETPSW_I,
    (void *)&INT_COMMON_NORTi_hook_entry_addr,
    (void *)&NORTi_INT_HOOK_KNL_LEVEL,
    (void *)__sectop("C$VECT")
};

// The code size of the following functions is just 4 bytes.
// For nonOS
// 00000000 39rrrr               W  BSR.W __$INT_COMMON_nonOS
// 00000003 02                      RTS // Not used.
// For NORTi
// 00000000 39rrrr               W  BSR.W __$INT_COMMON_NORTi
// 00000003 02                      RTS // Not used.
